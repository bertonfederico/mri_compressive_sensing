...
$$
J(x) = \frac{1}{2} \| \text{mask} \cdot (\text{FFT2D}(x) - y_{\text{init}}) \|_2^2
$$
...
$$
\nabla_x J(x) = \text{IFFT2D}(\text{mask} \cdot (\text{FFT2D}(x) - y_{\text{init}}))
$$
...
$$
x^{(k+1)} = \text{wavelet}_T(S_\lambda(z^{(k+1)}))
$$
...
$$
\min_{(x,z)} \frac{1}{2} \| \text{mask} \cdot (\text{FFT2D}(x) - y) \|_2^2 + \lambda \| W(x) \|_1
$$

In augmented Lagrangian form, it is expressed as:

$$
L_\rho(x, z, u) = \frac{1}{2} \| \text{mask} \cdot (\text{FFT2D}(x) - y) \|_2^2 + \lambda \| W(z) \|_1 + \frac{\rho}{2} \| z - x - u \|_2^2
$$
...
$$
x^{(k+1)} = \arg \min_x \left( \frac{1}{2} \| \text{mask} \cdot (\text{FFT2D}(x) - y) \|_2^2 + \frac{\rho}{2} \| z^k - x - u^k \|_2^2 \right)
$$
...
$$
\min_{(x,z)} \frac{1}{2} \| \text{mask} \cdot (\text{FFT2D}(x) - k) \|_2^2 + \lambda E_{\text{TV}}(z) \quad \text{s.t.} \quad x = z
$$
